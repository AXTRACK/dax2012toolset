...
namespace DEV.AX.EditorExtensions
{
    ...
    {
        private string[] startTokens = new string[] { "{", "/*", @"//region", "[" };
        private string[] endTokens = new string[] { "}", "*/", @"//endregion", "]" };
        ...
        private void LoadSettings()
        {
            ...
            EnableOL = settings.GeneralSettings.EnableOL;
        }
        ...
        void ReParse()
        {
            ...
            foreach (var line in newSnapshot.Lines)
            {
                ...
                foreach (string token in startTokens)
                {
                    regionStart = -1;
                    if ((regionStart = text.IndexOf(token, StringComparison.OrdinalIgnoreCase)) != -1)
                    {
                        type = "S";
                ...
                {
                    idx = 0;
                    foreach (string token in endTokens)
                    {
                        regionStart = -1;
                        if ((regionStart = text.IndexOf(token, StringComparison.OrdinalIgnoreCase)) != -1)
                        {
